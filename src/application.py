...
from config import States # Возможные состояния пользователей
...
@bot.message_handler(func=lambda msg: not users_db.is_user(msg.from_user.id) or
                        msg.text == 'Главное меню')
@bot.message_handler(commands=['start'])
def start_menu(msg):
    bot.send_message(msg.chat.id, 'Выберите пункт меню:',
...
def timetable(msg):
    bot.send_message(msg.chat.id, 'Введите группу:',
                     reply_markup=keyboard.standard())
...
def sub_timetable(msg):
    if len(subs_db.get_groups(msg.from_user.id)) == 0:
        bot.send_message(msg.chat.id, 'У вас нет подписок!',
                         reply_markup=keyboard.standard())
    else:
...
def sub_group_timetable(msg):
    ...
    # вывод расписания на завтра
        ...
        if subs_db.is_subscribed(msg.from_user.id, group_name):
            ...
            bot.send_message(msg.chat.id,
                             f'Вы не подписаны на группу {group_name}')
        return
