...
def parse_energy():
    url = 'https://www.victoria-group.ru/catalog/voda_soki_napitki/energeticheskie_napitki/'
...
def parse_bread():
    url = 'https://www.victoria-group.ru/catalog/khleb_torty_sladosti/khleb_bulochki_lepeshki_lavash_1/'
    html = get_html(url)
    if (html.status_code == 200): # все ОК
        # получить контент страницы
        goods = []
...
##############################################################################################
# эту часть можно будет в итоге удалить
...
def test():
    ...
    goods.extend(parse_energy())
    goods.extend(parse_bread())
